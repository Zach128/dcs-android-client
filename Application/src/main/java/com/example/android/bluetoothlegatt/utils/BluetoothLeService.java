...
public class BluetoothLeService extends Service {
    ...
    /**
     * Intent wrapper for broadcasting status updates.
     * Used for broadcasting updates on characteristic reading and is hence mainly used for data parsing.
     * @param action
    ...
    public void readCICCharacteristic() {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            Log.w(TAG, "BluetoothAdapter not initialized");
            return;
        }
        ...
        BluetoothGattService mDcsService = mBluetoothGatt.getService(UUID_DEVICE_CONTROL_SERVICE);
        if(mDcsService == null){
        ...
        BluetoothGattCharacteristic mCicCharacteristic = mDcsService.getCharacteristic(UUID_CONTROL_INSTRUCTION_CHARACTERISTIC);
        if(mBluetoothGatt.readCharacteristic(mCicCharacteristic) == false){
            Log.w(TAG, "Failed to read CIC characteristic");
        }
    }

    public void readIacCharacteristic() {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            Log.w(TAG, "BluetoothAdapter not initialized");
            return;
        }
        ...
        /*get the read characteristic from the service*/
        BluetoothGattCharacteristic mIacCharacteristic = mDcsService.getCharacteristic(UUID_INSTRUCTION_ARGUMENT_CHARACTERISTIC);
